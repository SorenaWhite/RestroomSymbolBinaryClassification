...
from mobilemlp.optim_factory import create_optimizer, LayerDecayValueAssigner
from mobilemlp.mobilemlp import MobileMLP_Small, MobileMLP_Large

from datasets.MMLRestroomSign import MMLRestroomSign, build_transform
from mobilemlp.engine import train_one_epoch, evaluate

from mobilemlp.utils import NativeScalerWithGradNormCount as NativeScaler
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='mobilemlp_large', type=str, metavar='MODEL', help='Name of model to train')
    parser.add_argument('--if_llm', default=False, type=bool, help='use llm extract features')
    parser.add_argument('--drop_path', type=float, default=0, metavar='PCT', help='Drop path rate (default: 0.0)')
    parser.add_argument('--input_size', default=224, type=int, help='image input size') # 224 256
    ...
    parser.add_argument('--resplit', type=str2bool, default=False, help='Do not random erase first (clean) augmentation split')
    ...
    parser.add_argument('--model_key', default='model|module', type=str, help='which key to load from saved state dict, usually model or model_ema')
    ...
    parser.add_argument('--pin_mem', type=str2bool, default=True, help='Pin CPU memory in DataLoader for more efficient (sometimes) transfer to GPU.')
...
def main(args):
    ...
    # prepare dataset
    dataset_train = MMLRestroomSign(args=args, transform=build_transform(args=args, is_train=True), is_train=True)
    dataset_val = MMLRestroomSign(args=args, transform=build_transform(args=args, is_train=False), is_train=False)
    ...
    if args.model == "mobilemlp_small":
        model = MobileMLP_Small()
    elif args.model == "mobilemlp_large":
    ...
    if mixup_fn is not None:
        ...
        criterion_label = SoftTargetCrossEntropy()
    elif args.smoothing > 0.:
        criterion_label = LabelSmoothingCrossEntropy(smoothing=args.smoothing)
    else:
        criterion_label = torch.nn.CrossEntropyLoss()
    criterion_feat = torch.nn.MSELoss()


    print("criterion = %s" % str(criterion_label))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, criterion_label, criterion_feat, data_loader_train, optimizer,
        ...
        if data_loader_val is not None:
            ...
            if args.model_ema and args.model_ema_eval:
                ...
                if max_accuracy_ema < test_stats_ema["acc1"]:
                    ...
                    if args.output_dir and args.save_ckpt: utils.save_model(     args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,     loss_scaler=loss_scaler, epoch="best-ema", model_ema=model_ema)
