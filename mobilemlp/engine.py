    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if use_amp:
            with torch.cuda.amp.autocast():
                ...
                male_target = torch.zeros((male_samples.shape[0], 1)).cuda()  # torch.zeros_like(male_output).float()
                female_target = torch.ones((female_samples.shape[0], 1)).cuda()  # torch.ones_like(female_output).float()
